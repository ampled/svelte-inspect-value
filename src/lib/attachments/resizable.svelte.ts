...
type ResizeCallbackFn = (w: number | undefined, h: number | undefined) => void

export const createResizable = (
  params: () => ResizeableParams,
  width: number | undefined,
  height: number | undefined,
  onDrag: ResizeCallbackFn,
  onResize: ResizeCallbackFn
) => {
  const { enabled, handles } = $derived(params())
  ...
  return {
    attach: (ele: HTMLElement) => {
      element = ele
      parentElement = ele.parentElement as HTMLElement
      ...
      $effect(() => {
        if (enabled) {
          ...
          let resetX = false
          let resetY = false
          if (!handles.includes('left') && !handles.includes('right')) {
            resetX = true
            width = undefined
          }
          if (!handles.includes('bottom') && !handles.includes('top')) {
            resetY = true
            height = undefined
          }
          ...
          untrack(() => {
            if (resetX || resetY) {
              onResize(resetX ? undefined : width, resetY ? undefined : height)
            }
          })
        } else {
          width = undefined
